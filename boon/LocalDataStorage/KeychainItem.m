...
- (id)initWithKey:(NSString *)key namespace:(NSString *)nameSpace {
    ...
    if (self) {
        ...
        _keychainItem = [KeychainItem keychainItemForKey:key service:_nameSpace];
        _key = key;
        [self reload];
    }
    return self;
...
- (OSStatus)save {
    ...
    if ([self exists]) {
        return [KeychainItem updateValue:_value inKeychainItem:_keychainItem];
    } else {
        return [KeychainItem setValue:_value inKeychainItem:_keychainItem];
    }
}

- (OSStatus)reload {
    OSStatus status = noErr;
    _value = [KeychainItem valueFromKeychainItem:_keychainItem statusCode:status];
    return status;
}

- (BOOL)exists {
    return [KeychainItem keychainContainsItem:_keychainItem];
}
...
+ (NSDictionary *)keychainItemForKey:(NSString *)key service:(NSString *)service {
    ...
    keychainItem[(__bridge id)kSecAttrAccessibleWhenUnlockedThisDeviceOnly] = (__bridge id)kSecAttrAccessibleWhenUnlockedThisDeviceOnly;
