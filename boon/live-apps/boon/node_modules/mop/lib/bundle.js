...
function bundle(appPackage, config) {
    config.bundledModules = config.bundledModules || [];
    // replace montage.js and mr/bootstrap.js references with bundles
    return Promise.each(Object.values(appPackage.files), function (file) {
    ...
    // TODO: handle package.json-specified serialization entry points
    .then(function () {
        config.out.status();
        return null;
    })
...
// searches for opportunities to bundle, then uses the appropriate bundler for
...
function bundleHtml(file, config) {
    ...
    return Promise.each(jobs, Promise.try);
}
...
function collectPreloadBundles(loader, bundler, config) {
    ...
    return Promise.each(batches, function (batch) {
        return Promise.each(batch, function (ids) {
...
function collectBundle(loader, bundler, config, bundle) {
    ...
    Object.entries(loader.packages).forEach(([location, loaderPackage]) => {
    ...
    Object.entries(loader.packages).forEach(([location, loaderPackage]) => {
        var bundlerPackage = bundler.getPackage(location);
        Object.entries(loaderPackage.modules).forEach(([id, loaderModule]) => {
            ...
            if (loaderModule.location !== void 0 && loaderModule.type === "javascript") {
                /*
                    module ids are cached/indexed as lowercase, so id is lowercase, but the file may not be, so we use loaderModule.id :
                */
                var relocation = URL.resolve(bundlerPackage.location, loaderModule.id + '.load.js'),
                    excludeExpression;
                if (config.files[relocation]) {
                    bundle.push(config.files[relocation].utf8);
                    excludeExpression = excludeExpressionForPath(config.files[relocation].buildLocation);
                    if (excludeExpression) {
                        config.bundledModules.push(excludeExpression);
                    }
                }
            }
        });
...
function excludeExpressionForPath (path) {
    var parts = path ? path.split("packages") : [],
        expression = null;

    if (parts.length === 2) {
        expression = "**/packages" + parts[1];
    }
...
function shardBundle(bundle, shardCount) {
    ...
    for (i = 0; i < bundle.length; i++) {
        ...
        if(part) {
            ...
            console.error(" FIX ME: a bundle shouldn't have an undefined part");
        }
    }
