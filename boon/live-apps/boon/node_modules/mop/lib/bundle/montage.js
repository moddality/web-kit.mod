...
var Require = require("montage/core/mr");
...
function bundleMontageHtml(element, file, config) {
    ...
    .then(function (loader) {
        ...
        .then(function (preloadPhases) {
            ...
            const filename = /[^\/]*$/.exec(file.buildLocation)[0];
            const base = file.document.createElement("base");
            base.setAttribute("href", file.buildLocation.slice(file.package.indexLocation.length, -filename.length));
            element.parentNode.insertBefore(base, element.parentNode.firstChild);
            file.buildLocation = file.package.indexLocation + filename;
        });
    });
}
...
function loadMontageScript(element, file, config) {
    ...
    .then(function (applicationPackage) {
        ...
        .spread(function(montagePackage, promisePackage) {
...
function collectMontageBootstrapBundle(loader, bundler, config) {
    ...
    loader.getPackage({name: "montage"}).getModuleDescriptor("bluebird").bundled = true;
    loader.getPackage({name: "bluebird"}).getModuleDescriptor("bluebird").bundled = true;
    loader.getPackage({name: "bluebird"}).getModuleDescriptor("js/browser/bluebird").bundled = true;
