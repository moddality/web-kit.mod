...
function read(location, config) {
    ...
    .then(function (appPackage) {
        return loadDeepPackages(appPackage, config)
        .then(function (pkg) {
...
function loadDeepPackages(package, config, memo) {
    memo = memo || {},
    devDependencies = package.packageDescription.devDependencies,
    packageLockDependencies = config.packageLock.dependencies,
    isProduction = config.production;

    var mappings = package.mappings || {};
    return Promise.all(Object.keys(mappings).map(function (id) {
        if(!packageLockDependencies[id] || !packageLockDependencies[id].dev || !devDependencies || (devDependencies && !devDependencies.hasOwnProperty(id)) || !isProduction) {
            ...
            if (!memo[location]) {
                ...
                .then(function (dependency) {
                    ...
                    return loadDeepPackages(dependency, config, memo);
                });
                memo[location] = loading;
                return loading;
            }
