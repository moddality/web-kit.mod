exports.format = (schema) => {
    ...
    return `DROP FUNCTION IF EXISTS "${schema}".anyarray_remove(anyarray, anyarray);
    CREATE OR REPLACE FUNCTION "${schema}".anyarray_remove(from_array anyarray, remove_array anyarray)
            RETURNS anyarray AS
    $BODY$
            DECLARE
    ...
    DROP FUNCTION IF EXISTS "${schema}".anyarray_remove(anyarray, anynonarray);
    CREATE OR REPLACE FUNCTION "${schema}".anyarray_remove(from_array anyarray, remove_element anynonarray)
            RETURNS anyarray AS
    $BODY$
            BEGIN
